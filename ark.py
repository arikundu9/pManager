...
def getRepoCodeAndName(entity):
    rc = "using pMan.DAL.Entities;\n"\
        "using pMan.DAL.Interfaces;\n"\
        "namespace pMan.DAL\n"\
        "{\n"\
        "   public class ||ENTITY||Repo : Repository<||ENTITY||, pManDBContext>, I||ENTITY||Repo\n"\
        "   {\n"\
        "       public ||ENTITY||Repo(pManDBContext context) : base(context)\n"\
        "       {\n"\
        "       }\n"\
        "   }\n"\
        "}"\
...
if args.m:
    ...
    model_code.append("    }\n")
    model_code.append("}")
    f.writelines(["namespace pMan.Models{\n"])
